/*
  ...
  Shortened bushing below swing arm so shaft collar wouldn't impact HDPE plate below
  Changed gramsec constraints from 0.4, 30.0 to gramsecmin and gramsec max (When using memphis red, it tries to output 30 g/s but can't keep up and impacts rod)
  Gramsecmax is recalculated upon calibration so it doesnt go past SNhigh, this will limit output range depending on specific media calibration
*/
...
float gramsecmin = 0.4;             //Minimum grams/second setting
float gramsecmax = 30.0;            //Maximum grams/second setting, will be recalculated upon calibration if curve indicates
float gramsecmax2 = 30.0;           //Saved gramsecmax value 
...
void calculate()  //Calculate A and B values for power regression of calibration values
{
  ...
  float tempgramsec = powA * (pow(SNhigh, powB));//y = Ax^B, g/s = powA * SN ^ powB, solved for SN
  if (tempgramsec < gramsecmax2)
  {
    gramsecmax = tempgramsec;
  }
}
...
void fetch()  //Fetch calibration settings from the EEPROM
{
  ...
  address += (sizeof(float));
  EEPROM.get(address, gramsecmax);
  if ((isnan(powA)) || (isnan(powB)))            //No value saved to address
  {
    powA = powA2;
    powB = powB2;
    gramsecmax = gramsecmax2;
    clearLCD();
    cursorHome();
    Serial.print(F("Calibration"));
    cursorLine2();
    ...
    case 2:
      ...
      while (menu == 2)
      {
        ...
        if (edit)   //Editing gram/sec
        {
          ...
          gramsec = constrain(gramsec, gramsecmin, gramsecmax);                //Constrain to working range
...
void setup()
{
  ...
  cursorLine2();
  Serial.print(F("Software v3.21.0"));
  delay(textDelay);
